...
- `fail`
  ...
  - /riakkv/latest/developing/usage/commit-hooks/
---
The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{<baseurl>}}riak/kv/2.9.10/developing/api/http)) along with a generic error message about why the write was blocked.
- `{fail, Reason}`
  - /riak/latest/developing/usage/commit-hooks/
  - /riak/kv/latest/developing/usage/commit-hooks/
  - /riakkv/latest/developing/usage/commit-hooks/
