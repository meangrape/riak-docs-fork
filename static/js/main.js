...
/* This pack was generate with the following languages;
Common:
        Apache Bash C# C++ CSS CoffeeScript Diff HTML/XML HTTP Ini JSON Java
        JavaScript Makefile Markdown Nginx Objective-C PHP Perl Python Ruby SQL
Other:
        Dart Dockerfile Elixir Erlang Erlang-REPL Go Haskell Matlab
        Protocol-Buffers Puppet SCSS Scala
*/
...
(function() {
  'use strict';
  window.padNumber = function(number, len, char) {
    ...
    if (char !== '0') {
      zeroString.replace(/^0+/, function(m) {
        return m.replace(/0/g, char);
      });
    }
    if (number < 0) {
  ...
  window.rem = function(n) {
    if (n == null) {
      n = 1;
    }
    return n * parseInt($('html').css('font-size'));
  };

  Number.prototype.rem = function() {
    return this * parseInt($('html').css('font-size'));
  };
...
(function() {
  'use strict';
  var SemVer, version_max, version_min;
  ...
  SemVer.parseRange = function(str) {
    var version, vs;
  ...
  SemVer.isInRange = function(version, range) {
    var newest, oldest;
    oldest = range[0], newest = range[1];
    if (SemVer.compare(version, oldest) > 0 || SemVer.compare(version, newest) < 0) {
      return false;
    } else {
      return true;
    }
...
(function() {
  $("main").each(function() {
    var headings, main;
    main = $(this);
    headings = main.find('h1').add(main.find('h2')).add(main.find('h3')).add(main.find('h4')).add(main.find('h5')).add(main.find('h6'));
    return headings.each(function() {
      var anchor, contents, id, that;
      that = $(this);
      id = that.attr('id');
      contents = that.html();
      anchor = "<a class='anchor-icon' href='#" + id + "'></a>";
      return that.wrapInner(anchor);
    });
  });
...
(function() {
  var h2s, toc, toc_items, toc_title, toc_wrapper;
  ...
  if (h2s.length >= 6) {
    toc_wrapper.addClass("table-of-contents__wrapper--multi");
  }

  h2s.each(function() {
    var $that;
    $that = $(this);
    return toc_items.append($("<li/>", {
...
(function() {
  $(function() {
    var content_nav_toggles, content_well, menu_items, side_nav;
    side_nav = $('.content-nav');
    content_well = $('.content-well');
    content_nav_toggles = $('.js_toggle-content-nav');
    menu_items = $('.content-menu__item');
    content_nav_toggles.on('click', function() {
      var content_well_top, window_scroll;
      ...
      if (content_well.hasClass('content-well--immobile')) {
        return content_well.css('top', -1 * window_scroll);
      } else {
        $(window).scrollTop(-1 * content_well_top);
        return content_well.css('top', '');
      }
    });
    menu_items.on('click', '.content-menu__menu-toggle', function() {
      var menu;
      menu = $(this.parentNode.nextElementSibling);
      if (menu.hasClass('content-menu--open')) {
        menu.slideUp('fast');
      } else {
        menu.slideDown('fast');
      }
      menu.toggleClass('content-menu--open');
      return $(this).toggleClass('content-menu__menu-toggle--open');
    });
  });
...
(function() {
  'use strict';
  var contentOfMeta, generateVersionLists;

  contentOfMeta = function(name) {
    return $('meta[name=' + name + ']').attr('content');
  };

  generateVersionLists = function() {
    var current_version, docs_root_url, locations_json, meta_version_history_in, meta_version_history_locations, path, project, project_descriptions_path, project_relative_path, range, version_range, versions_locations;
    project = contentOfMeta("project");
    current_version = contentOfMeta("version");
    project_relative_path = contentOfMeta("project_relative_path");
    if (!project_relative_path) {
      project_relative_path = "";
    }
    docs_root_url = contentOfMeta("docs_root_url");
    if (!docs_root_url) {
      docs_root_url = "";
    }
    project_descriptions_path = docs_root_url + 'data/project_descriptions.json';
    ...
    if (meta_version_history_locations) {
      ...
      versions_locations = (function() {
        var i, len, ref, results;
        results = [];
        for (i = 0, len = locations_json.length; i < len; i++) {
          ref = locations_json[i], range = ref[0], path = ref[1];
          results.push([SemVer.parseRange(range), path]);
        }
        return results;
      })();
    }
    if (project && project !== "community" && project !== "404") {
      return $.getJSON(project_descriptions_path, function(data) {
        var $version_pane, $version_pane__shadow, $version_pane__sizing, anchor, anchor_tag, archived_url, arrow_str, class_list, i, in_version_range, index, j, k, latest_rel, len, len1, len2, list_depth, lts_series, project_data, project_path, ref, ref1, ref2, relative_path, release_sem_ver, release_set, release_version, set_index, tallest_list, url, version_selector_list_html;
        project_data = data[project];
        project_path = docs_root_url.replace(/\/+$/, '') + project_data.path;
        latest_rel = project_data.latest;
        lts_series = project_data['lts'];
        archived_url = project_data['archived_url'];
        version_selector_list_html = '';
        ref = project_data.releases.reverse();
        for (set_index = i = 0, len = ref.length; i < len; set_index = ++i) {
          ...
          if (lts_series && release_set[0].match("^" + lts_series)) {
            class_list = ["selector-list__element", "selector-list__element--" + list_depth, "selector-list__element--lts-flag"];
            version_selector_list_html += '<li class="' + class_list.join("\n") + '">' + '<a class="block">LTS</a>' + '</li>';
          }
          ref1 = release_set.reverse();
          for (index = j = 0, len1 = ref1.length; j < len1; index = ++j) {
            release_version = ref1[index];
            release_sem_ver = SemVer.parse(release_version);
            in_version_range = !version_range || SemVer.isInRange(release_sem_ver, version_range);
            ...
            if (in_version_range && (release_version !== current_version)) {
              relative_path = project_relative_path;
              for (k = 0, len2 = versions_locations.length; k < len2; k++) {
                ref2 = versions_locations[k], range = ref2[0], url = ref2[1];
        ...
        if (archived_url) {
          class_list = ["selector-list__element", "selector-list__element--archived"];
        ...
        tallest_list = Math.max.apply(Math, $version_pane.find('.selector-list').map(function() {
          return $(this).outerHeight();
        }));
        $version_pane__shadow.css('height', tallest_list + 2..rem());
        $version_pane__sizing.css('height', tallest_list + 2..rem());
        $('.selector-pane--versions').find('.js_edge-fader--target').on('scroll.selector-fader-target', throttle(function(event) {
          return EdgeFader.verifyArrowState($(this));
        }, 250));
        return $('.edge-fader__arrow').on('click.selector-fader-arrow', EdgeFader.onClickCallback);
      });
    }
  };

  $(function() {
    var $content_nav, $content_nav__primary, $version_pane, $version_pane__shadow, $version_pane__sizing, $version_selector, debouced_show_or_hide_selector_list_arrows;
    ...
    $version_pane__shadow = $version_pane.parent();
    $version_pane__sizing = $version_pane__shadow.parent();
    debouced_show_or_hide_selector_list_arrows = debounce(function() {
      if ($version_pane.closest('.selector-pane__sizing-box--hidden').length === 0) {
        return $version_pane.find('.selector-list__scroll-box').each(function(index) {
          return EdgeFader.showOrHideArrows($(this));
        });
      }
    }, 500);
    $version_selector.on('click.toggle_selector', '.selector__btn', function(event) {
      var $selector;
      ...
      if ($selector.hasClass('selector--open')) {
        ...
        return debouced_show_or_hide_selector_list_arrows();
      } else {
        $version_pane__sizing.css('max-height', '');
        EdgeFader.hideArrows($version_pane);
        return $version_pane.find('.selector-list__scroll-box').each(function(index) {
          return EdgeFader.hideArrows($(this));
        });
      }
    });
    $(window).on('resize.toggled_selector_reize', debounce(function(event) {
      var tallest_list;
      tallest_list = Math.max.apply(Math, $version_pane.find('.selector-list').map(function() {
        return $(this).outerHeight();
      }));
    ...
    return $(document).on('click.selector_close', function(event) {
      if ($(event.target).closest('.selector-pane__sizing-box').length > 0) {
        return true;
      }
      if ($(event.target).hasClass('selector__btn')) {
        return true;
      }
...
(function() {
  ...
  EdgeFader.verifyArrowState = function($target) {
    var $bottom_arrow, $left_arrow, $right_arrow, $top_arrow, $wrapper, current_left, current_top, max_left, max_top;
  ...
  EdgeFader.showOrHideArrows = function($target) {
    var $bottom_arrow, $left_arrow, $right_arrow, $top_arrow, $wrapper, max_left, max_top;
    $wrapper = $target.parent();
    $left_arrow = $wrapper.children('.edge-fader--left').children('.edge-fader__arrow');
    $right_arrow = $wrapper.children('.edge-fader--right').children('.edge-fader__arrow');
    $top_arrow = $wrapper.children('.edge-fader--top').children('.edge-fader__arrow');
  ...
  EdgeFader.showArrows = function($target) {
    var $bottom_arrow, $left_arrow, $right_arrow, $top_arrow, $wrapper;
    $wrapper = $target.parent();
    $left_arrow = $wrapper.children('.edge-fader--left').children('.edge-fader__arrow');
    $right_arrow = $wrapper.children('.edge-fader--right').children('.edge-fader__arrow');
    $top_arrow = $wrapper.children('.edge-fader--top').children('.edge-fader__arrow');
    ...
    $right_arrow.removeClass('edge-fader__arrow--invisible');
    $top_arrow.removeClass('edge-fader__arrow--invisible');
    return $bottom_arrow.removeClass('edge-fader__arrow--invisible');
  };

  EdgeFader.hideArrows = function($target) {
    var $bottom_arrow, $left_arrow, $right_arrow, $top_arrow, $wrapper;
    $wrapper = $target.parent();
    $left_arrow = $wrapper.children('.edge-fader--left').children('.edge-fader__arrow');
    $right_arrow = $wrapper.children('.edge-fader--right').children('.edge-fader__arrow');
    ...
    $right_arrow.addClass('edge-fader__arrow--invisible');
    $top_arrow.addClass('edge-fader__arrow--invisible');
    return $bottom_arrow.addClass('edge-fader__arrow--invisible');
  };

  EdgeFader.onClickCallback = function(event) {
    var $arrow, $fader, $target, $wrapper, new_pos;
    $arrow = $(this);
    $fader = $arrow.parent();
  ...
  $(function() {
    $('.js_edge-fader--target').on('scroll.edge-fader-target', throttle(function(event) {
      return EdgeFader.verifyArrowState($(this));
    }, 250));
    $(window).on('resize.edge-fader-target', throttle(function(event) {
      return $('.edge-fader').parent().children('.js_edge-fader--target').each(function() {
        return EdgeFader.showOrHideArrows($(this));
      });
    }, 250));
    $('.edge-fader__arrow').on('click.edge-fader-arrow', EdgeFader.onClickCallback);
  });
...
(function() {
  'use strict';
  var getLanguage, language_transforms;
  ...
  language_transforms = {
    ...
    'language-riakconf': {
      display_name: 'riak.conf',
      highlight_as: 'language-matlab'
    },
    'language-riakcsconf': {
      display_name: 'riak-cs.conf',
      highlight_as: 'language-matlab'
    },
    'language-stanchionconf': {
      display_name: 'stanchion.conf',
      highlight_as: 'language-matlab'
    },
    'language-vmargs': {
      display_name: 'vm.args',
      highlight_as: 'language-ini'
    },
    'language-bash': {
      display_name: 'Shell',
      highlight_as: ''
  ...
  getLanguage = function($element) {
    var ref, ref1;
    return (ref = $element.attr('class')) != null ? (ref1 = ref.match(/(?:^|\s)(language-.+?)(?:\s|$)/)) != null ? ref1[1] : void 0 : void 0;
  };
  ...
  $('pre > code').each(function(index) {
    ...
    if (language) {
      ...
      if (language in language_transforms) {
        highlight_as = (ref = language_transforms[language]) != null ? ref.highlight_as : void 0;
        if (highlight_as) {
          $code.removeClass(language);
    ...
    if ($pre.parent().hasClass('code-block__code-set')) {
      return;
    }
    ...
    if ($siblings.length) {
      return $pre.add($siblings).wrapAll('<div class="code-block--tabbed"><div class="code-block__code-set">');
    }
  });

  $('.code-block--titled').each(function(index) {
    var $code, $code_block, $pre, display_name, language, ref;
    $code_block = $(this);
    $pre = $code_block.children('pre');
    $code = $pre.children('code');
    language = getLanguage($code);
    $pre.addClass("code-block__code");
    display_name = (ref = language_transforms[language]) != null ? ref.display_name : void 0;
    if (!display_name) {
      display_name = language.replace(/language-/, '');
    }
    return $('<span class="inline-block   code-block__title">' + display_name + '</span>').prependTo($code_block);
  });

  $('.code-block--tabbed').each(function(code_block_index) {
    var $code_set, $tab_set, $tab_set_wrapper, $tabbed_code_block, arrow_str;
    $tabbed_code_block = $(this);
    $code_set = $tabbed_code_block.children('.code-block__code-set');
    arrow_str = '<span class="inline-block   edge-fader__arrow edge-fader__arrow--invisible"></span>';
    $tab_set_wrapper = $('<div class="code-block__tab-set-wrapper">' + '<div class="edge-fader edge-fader--left ">' + arrow_str + '</div>' + '<div class="edge-fader edge-fader--right">' + arrow_str + '</div>' + '</div>');
    $tab_set = $('<ul class="overflow-x   code-block__tab-set  js_edge-fader--target">');
    $tab_set.appendTo($tab_set_wrapper);
    $code_set.children('pre').each(function(code_index) {
      var $code, $pre, code_id, data_lang, display_name, language, ref;
      $pre = $(this);
      $code = $pre.children();
      language = getLanguage($code);
      display_name = (ref = language_transforms[language]) != null ? ref.display_name : void 0;
      if (!display_name) {
        display_name = language != null ? language.replace(/language-/, '') : void 0;
      }
      if (!display_name) {
        display_name = "////" + padNumber(code_index, 2);
      }
      data_lang = language;
      if (!data_lang) {
        data_lang = "unnamed-lang" + padNumber(code_index, 2);
      }
      code_id = "code-block__" + data_lang + padNumber(code_index, 3);
      $pre.addClass('code-block__code');
      $pre.attr('id', code_id);
      return $tab_set.append('<li class="inline-block   code-block__tab">' + '<a class="block" ' + 'href="#' + code_id + '" ' + 'data-language="' + data_lang + '">' + display_name + '</a>' + '</li>');
    });
    $tab_set.find('.code-block__tab').first().addClass('code-block__tab--active');
    $tabbed_code_block.find('.code-block__code').first().addClass('code-block__code--active');
    $tabbed_code_block.prepend($tab_set_wrapper);
    return EdgeFader.showOrHideArrows($tab_set);
  });

  $(function() {
    var $tabbed_code_blocks;
    $tabbed_code_blocks = $('.code-block--tabbed');
    $('.code-block__tab').on('click.code-block-tab', 'a', function(event) {
      var $anchor, $code_block, $event_anchor, element, i, language, len, top_offset;
      event.preventDefault();
      $event_anchor = $(this);
      language = $event_anchor.data('language');
      if ($event_anchor.parent().hasClass('code-block__tab--active')) {
        return;
      }
      top_offset = this.getBoundingClientRect().top;
      for (i = 0, len = $tabbed_code_blocks.length; i < len; i++) {
        element = $tabbed_code_blocks[i];
        $code_block = $(element);
        $anchor = $code_block.find('[data-language="' + language + '"]');
        if ($anchor.length === 0) {
          continue;
        }
        $code_block.find('.code-block__tab--active').removeClass('code-block__tab--active');
        $anchor.parent().addClass('code-block__tab--active');
        $code_block.find('.code-block__code--active').removeClass('code-block__code--active').hide();
        $code_block.find($anchor.attr('href')).addClass('code-block__code--active').fadeIn();
      }
